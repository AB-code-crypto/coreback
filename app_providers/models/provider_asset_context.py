...
class ProviderAssetContext(UUIDTimestampedModel):
    ...
    is_active = models.BooleanField(
        ...
        help_text="Используется ли эта запись в системе.",
    )
    ...
    asset_code_pl = models.CharField(
        max_length=64,
        blank=True,
        default="",
        db_index=True,
        verbose_name="Тикер актива (raw)",
        help_text="Как код актива пришёл от провайдера.",
    )
    asset_name_pl = models.CharField(
        max_length=255,
        blank=True,
        default="",
        verbose_name="Название актива (raw)",
        help_text="Как название актива пришло от провайдера.",
    )

    context_code_pl = models.CharField(
        max_length=128,
        blank=True,
        default="",
        db_index=True,
        verbose_name="Код контекста (raw)",
        help_text="Как код сети/контекста пришёл от провайдера.",
    )
    context_name_pl = models.CharField(
        max_length=255,
        blank=True,
        default="",
        verbose_name="Название контекста (raw)",
        help_text="Как название сети/контекста пришло от провайдера.",
    )

    contract_raw = models.CharField(
        max_length=256,
        blank=True,
        null=True,
        db_index=True,
        verbose_name="Адрес контракта",
        help_text="Контракт или иной технический идентификатор, если провайдер его отдаёт.",
    )

    raw_metadata = models.JSONField(
        default=dict,
        blank=True,
        verbose_name="Сырые метаданные",
        help_text="Дополнительные необработанные данные от провайдера.",
    )
    ...
    asset_code = models.CharField(
        ...
        default="",
        db_index=True,
        verbose_name="Тикер актива",
        help_text="Нормализованный тикер для поиска и логики.",
    )
    asset_name = models.CharField(
        ...
        help_text="Нормализованное отображаемое название актива.",
    )
    ...
    context_code = models.CharField(
        ...
        default="",
        db_index=True,
        verbose_name="Код контекста",
        help_text="Нормализованный код сети/контекста для поиска и логики.",
    )
    context_name = models.CharField(
        max_length=255,
        blank=True,
        default="",
        verbose_name="Название контекста",
        help_text="Нормализованное отображаемое название сети/контекста.",
    )

    # ---------------------------------------------------------
    # КЛАСТЕРИЗАЦИЯ / МАТЧИНГ
    # ---------------------------------------------------------

    cluster_no = models.PositiveIntegerField(
        null=True,
        blank=True,
        default=None,
        db_index=True,
        verbose_name="Кластер №",
        help_text="Номер кластера. Проставляется позже автоматически или вручную.",
    )

    is_front = models.BooleanField(
        default=False,
        db_index=True,
        verbose_name="На фронт",
        help_text=(
            "Запись-представитель кластера для фронта. "
            "В одном кластере допускается не более одной такой записи."
        ),
    )

    match_status = models.CharField(
        max_length=32,
        choices=ProviderAssetContextMatchStatus.choices,
        default=ProviderAssetContextMatchStatus.NEW,
        db_index=True,
    ...
    D = models.BooleanField(
        default=True,
        verbose_name="Ввод (ручной)",
        help_text="Ручное разрешение или запрет ввода.",
    )
    W = models.BooleanField(
        default=True,
        verbose_name="Вывод (ручной)",
        help_text="Ручное разрешение или запрет вывода.",
    )
    AD = models.BooleanField(
        default=True,
        verbose_name="Ввод (авто)",
        help_text="Автоматический статус ввода, полученный от провайдера.",
    )
    AW = models.BooleanField(
        default=True,
        verbose_name="Вывод (авто)",
    ...
    deposit_fee_fixed = models.DecimalField(
        ...
        help_text="Комиссия в единицах самого актива. Может быть отрицательной.",
    )
    deposit_fee_percent = models.DecimalField(
        max_digits=PERCENT_MAX_DIGITS,
        decimal_places=PERCENT_DECIMAL_PLACES,
        default=Decimal("0"),
        validators=[PERCENT_MAX_100],
        verbose_name="Комиссия на ввод, %",
        help_text="Процентная комиссия. Может быть отрицательной, но не может быть больше 100%.",
    )
    deposit_fee_min_amount = models.DecimalField(
        ...
        help_text="Минимальная комиссия на ввод в единицах актива. Может быть отрицательной.",
    )
    ...
    withdraw_fee_min_amount = models.DecimalField(
        ...
        help_text="Минимальная комиссия на вывод в единицах актива. Может быть отрицательной.",
    )
    ...
    is_stablecoin = models.BooleanField(
        default=False,
        db_index=True,
        verbose_name="Стейблкоин",
    )
    amount_precision = models.PositiveSmallIntegerField(
        default=8,
        verbose_name="Точность актива",
        help_text="Сколько знаков после запятой допустимо для этого инструмента.",
    )
    nominal = models.PositiveIntegerField(
        default=1,
        verbose_name="Номинал",
        help_text="Номинал инструмента. Для большинства активов = 1.",
    )
    ...
    reserve_current = models.DecimalField(
        ...
        default=Decimal("0"),
        validators=[NON_NEGATIVE_DECIMAL],
        verbose_name="Текущий резерв",
        help_text="Текущий доступный резерв у провайдера по этой записи.",
    )
    reserve_min = models.DecimalField(
        ...
        default=Decimal("0"),
        validators=[NON_NEGATIVE_DECIMAL],
        verbose_name="Мин. резерв",
        help_text="Минимальный допустимый резерв для логики маршрутизации.",
    )
    reserve_max = models.DecimalField(
        ...
        default=Decimal("0"),
        validators=[NON_NEGATIVE_DECIMAL],
        verbose_name="Макс. резерв",
        help_text="Максимальный допустимый резерв, если вы это используете в своей логике.",
    )
    ...
    icon_file = models.ImageField(
        upload_to="provider_asset_context_icons/",
        blank=True,
        null=True,
    ...
    icon_url = models.URLField(
        blank=True,
        default="",
        verbose_name="Иконка (URL)",
        help_text="Иконка, которую отдал провайдер или которую вы указали вручную.",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["provider", "asset_code", "context_code"]),
            models.Index(fields=["contract_raw"]),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["cluster_no"],
                condition=Q(is_front=True, cluster_no__isnull=False),
                name="uniq_front_provider_asset_context_per_cluster",
            ),
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        # имена мягко нормализуем без upper(), чтобы не портить отображение
